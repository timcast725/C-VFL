...
results.write(f'Accuracy reached after {667*min_bits_per_epoch} bits sent\n')
results.write('Max Std\n')

resultsMB = open('resultsMB.txt','w')
resultsMB.write('Avg Std\n')

def all_seeds(prefix, result_out, num_clients, bits, local_epochs=10):
    files = glob.glob(f'{prefix}_seed*.pkl')
    pickles = []    
    print(f'{prefix}_seed')
    for f in files:
    ...
    if local_epochs == 25:
        pickles = np.array([p[:400] for p in pickles])
    ...
    if result_out is not None:
        final_epoch = 0
        bits_so_far = 0
        while bits_so_far < 20*8*2**20: # 30 MB
            bits_so_far += bits_per_epoch
            final_epoch += 1
        ind = np.argmax(avg[0:final_epoch+1])
        max_ind = np.argmax(avg)
        if local_epochs > 10:
            max_ind = np.argmax(avg[:int(1000*10/local_epochs)])
        max_error = std[max_ind] 
        best_error = std[ind] 
        #result_out.write(f'& {avg[max_ind]:.3f} $\pm$ {max_error:.3f} & {avg[ind]:.3f} $\pm$ {best_error:.3f}\n')
        result_out.write(f'& {avg[max_ind]:.3f} $\pm$ {max_error:.3f} \n')

        bits_to_reach = []
        for i in range(len(pickles)):
            final_epoch = 0
            bits_so_far = 0
            while pickles[i][final_epoch] < 0.4:
                bits_so_far += bits_per_epoch
                final_epoch += 1
...
for t in types:
    ...
    if t == 'accs_test':
        f = results
        losses0 = all_seeds(f'{t}_varlr_BS1000_NC4_LE10_Q0_EFalse_dim1_comp',f,4,32)
    
    for q in Qs:
        bits = math.log(q,2)
        # Parse results
        losses0 = all_seeds(f'{t}_varlr_BS1000_NC4_LE10_Q0_EFalse_dim1_comp',None,4,32)
        losses1 = all_seeds(f'{t}_varlr_BS1000_NC4_LE10_Q{q}_EFalse_dim1_compquantize',f,4,bits)
        losses2 = all_seeds(f'{t}_varlr_BS1000_NC4_LE10_Q{q}_EFalse_dim2_compquantize',f,4,bits)
        losses3 = all_seeds(f'{t}_varlr_BS1000_NC4_LE10_Q{q}_EFalse_dim1_comptopk',f,4,bits)
        ...
        plt.fill_between(np.linspace(0,1000,1001), losses0[0] - losses0[1], losses0[0] + losses0[1], alpha=0.3)
        plt.fill_between(np.linspace(0,1000,1001), losses1[0] - losses1[1], losses1[0] + losses1[1], alpha=0.3)
        plt.fill_between(np.linspace(0,1000,1001), losses2[0] - losses2[1], losses2[0] + losses2[1], alpha=0.3)
        plt.fill_between(np.linspace(0,1000,1001), losses3[0] - losses3[1], losses3[0] + losses3[1], alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(f'{t}_quantcomm_{q}.png')
        #plt.show()
    
    if t == 'accs_test':
        losses_LE1 = all_seeds(f'{t}_varlr_BS1000_NC4_LE1_Q8_EFalse_dim2_compquantize',f,4,3,1)
        losses_LE25 = all_seeds(f'{t}_varlr_BS1000_NC4_LE25_Q8_EFalse_dim2_compquantize',f,4,3,25)
        losses_NC12 = all_seeds(f'{t}_varlr_BS1000_NC12_LE10_Q8_EFalse_dim2_compquantize',f,12,3)
