...
def all_seeds(prefix, result_out, num_clients, bits, local_epochs=10):
    ...
    if result_out is not None:
        ...
        if local_epochs > 10:
            max_ind = np.argmax(avg[:int(200*10/local_epochs)])
        ...
        for i in range(len(pickles)):
            ...
            while pickles[i][final_epoch] < 70:
                ...
                if final_epoch >= len(pickles[i]):
                    break
            if final_epoch >= len(pickles[i]):
                bits_to_reach.append(float('inf'))
            else:
...
for t in types:
    ...
    if t == 'accs_test':
        f = results
        losses0 = all_seeds(f'{t}_cifar_NC4_LE10_quant0_dim1_comp',f,4,32)
    
    for q in Qs:
        bits = math.log(q,2)
        # Parse results
        losses0 = all_seeds(f'{t}_cifar_NC4_LE10_quant0_dim1_comp',None,4,32)
        losses1 = all_seeds(f'{t}_cifar_NC4_LE10_quant{q}_dim1_compquantize',f,4,bits)
        losses2 = all_seeds(f'{t}_cifar_NC4_LE10_quant{q}_dim2_compquantize',f,4,bits)
        losses3 = all_seeds(f'{t}_cifar_NC4_LE10_quant{q}_dim1_comptopk',f,4,bits)
        ...
        plt.fill_between(np.linspace(0,200,201), losses0[0] - losses0[1], losses0[0] + losses0[1], alpha=0.3)
        plt.fill_between(np.linspace(0,200,201), losses1[0] - losses1[1], losses1[0] + losses1[1], alpha=0.3)
        plt.fill_between(np.linspace(0,200,201), losses2[0] - losses2[1], losses2[0] + losses2[1], alpha=0.3)
        plt.fill_between(np.linspace(0,200,201), losses3[0] - losses3[1], losses3[0] + losses3[1], alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(f'{t}_quant_cifar{q}.png')
        #plt.show()
        ...
        x_axis_bits = np.linspace(0, 200*min_bits_per_epoch/(8*2**30), min_inds)
        ...
        plt.tight_layout()
        plt.savefig(f'{t}_quantcomm_{q}_cifar.png')
        #plt.show()
